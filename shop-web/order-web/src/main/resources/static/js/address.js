...
//					禁止遮罩层下面的内容滚动
    ...
    $(".control").click(function () {
        ...
        for(var i=0; i<provinceList.length; i++){　　　　//provinceList.length为省数组的长度，下标从0开始，所以定义var i=0
            ...
            $("#province option:last-child").after("<option value='"+provinceName+"'>"+provinceName+"</option>");　　　　//通过Option方法将省的名字与下标i对应，取出来。然后通过add()方法，将每一个名字放到provinceTag中
        }
    })
    ...
    $("#province").change(function () {
        ...
        for(var n=0; n<provinceList.length; n++){　　　　//通过循环遍历列表数组
            ...
            if (provice.name == $(this).val()){　　　　　　//开始建立连接，通过if判断，前提是要满足if中的条件
                cityList = provice.cityList;　　　　　　　　　　　　//通过province的cityList获取城市列表
                $("#city option").remove();　　　　　　　　　　　//这个步骤很重要，分析语句，是吧里面的内容变为空，意思就是当你选择一个省时，后面会出现城市的数据，当你选择另外的省时会                                                                            出现相应的市的数据，如果不删除就会同时显示在一个选择框中，就不会是我们想要的结果
                for(var c=0; c<cityList.length; c++){　　　　　　//使用循环遍历城市数列
                    ...
                    $("#city").append("<option value='"+cityName+"'>"+cityName+"</option>");　　　　//通过Option方法将省的名字与下标i对应，取出来。然后通过add()方法，将每一个名字放到provinceTag中
                }
            }
        }
    })
    ...
    $("#city").change(function () {
        ...
        for(var j=0;j<cityList.length;j++){
            var city = cityList[j];
            if(city.name == $(this).val()){
                var areaList = city.areaList;
                $("#area option").remove();
                for( var k=0;k<areaList.length;k++){
                    var area = areaList[k];
                    $("#area").append("<option value='"+area+"'>"+area+"</option>");　　　　//通过Option方法将省的名字与下标i对应，取出来。然后通过add()方法，将每一个名字放到provinceTag中
                }
            }
        }
    })
